...
int main() {
	...
	char zelle;
	int durchgang = 0;
	zellen = malloc(GRÖSSE * sizeof(int*));
	for (int i = 0; i < GRÖSSE; i++) {
		zellen[i] = malloc(GRÖSSE * sizeof(int));
		if (NULL == zellen[i]) {
	...
	pzellen = malloc(GRÖSSE * sizeof(int*));
	for (int i = 0; i < GRÖSSE; i++) {
		pzellen[i] = malloc(GRÖSSE * sizeof(int));
		if (NULL == pzellen[i]) {
			printf("Kein Speicher mehr fuer Zeile %d\n", i);
			return EXIT_FAILURE;
		}
...
char** GetRandomStartZustand(char** zellen, int prozent) {
	int maxAnzahlAnLebendenZelle = ((GRÖSSE)*(GRÖSSE)) * ((float)(prozent) / 100);
	int maxToteZellen = ((GRÖSSE) * (GRÖSSE)) - maxAnzahlAnLebendenZelle;
	char zustand = ' ';
